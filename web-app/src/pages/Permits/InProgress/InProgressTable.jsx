...
const generateProofLetter = (permit) => {
  // Create a unique reference number
  const uniqueReference = `REF-${Date.now()}`;
  ...
  doc.setFontSize(12);
  doc.text("Proof of Claim Letter", 105, 20, { align: "center" });
  doc.setFontSize(10);
  doc.text(`Reference Number: ${uniqueReference}`, 20, 40);
  doc.text(
    `Resident Name: ${formatUserName(permit.resident.user, false)}`,
    20,
    50
  );
  doc.text(`Approval Date: ${permit.approval_date}`, 20, 60);
  doc.text(`Purpose: ${permit.purpose}`, 20, 70);
  doc.text(`Permit Status: Completed`, 20, 80);
  doc.text(
    "This letter serves as proof that the permit you requested has been marked as completed.",
    20,
    100,
    { maxWidth: 170 }
  );
  ...
  // Footer
  doc.setFontSize(8);
  doc.text("Generated by the Subdivision Management System", 105, 290, {
    align: "center",
  });

  // Save the document as a PDF
  doc.save(`Proof_of_Claim_${uniqueReference}.pdf`);
};
  ...
  const handleMarkAsDone = async (permit, e) => {
    ...
    if (confirm) {
      ...
      if (response) {
        ...
        generateProofLetter(permit); // Call the function to generate the proof
