...
import ClearanceIncomeDetails from "./ClearanceIncomeDetails";

const ClearanceIncome = ({ loading, incomes, error }) => {
    ...
    <div className="p-4 bg-lime-50 rounded-lg shadow-sm w-full max-w-xs">
      ...
      <div className="flex items-center justify-between mb-3">
        {loading ? (
          ...
          <div className="text-xl font-bold text-gray-800">
            {incomes && incomes.permits
              ? formatIncome(incomes.permits)
              : "₱0.00"}
          </div>
        )}
      </div>
      ...
      {showDetails && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <ClearanceIncomeDetails onClose={toggleDetails} />
        </div>
      )}
    </div>
  );
